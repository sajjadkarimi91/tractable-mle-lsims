...
function [scores_lsim, scores_hmm, scores_chmm, scores_svm, Label] = sim_calassification_chmm(C,T,train_number, test_number)
...
for s = 1:length(state_numbers_all)
    ...
    parfor reinit_num = 1:max_reinit
        ...
        [pi_0_chmm_temp , transition_matrices_convex_comb_temp ,  chmm_gmm_para_temp ,  AIC, log_likelyhood , BIC ,pi_steady] = ...
            em_chmm( train_obs_chmm_1 , Channel_Num_States_t , num_gmm_component_temp , max_itration , extra);
...
for s = 1:length(state_numbers_all)
    ...
    for reinit_num = 1:max_reinit
        ...
        [pi_0_chmm_temp , transition_matrices_convex_comb_temp ,  chmm_gmm_para_temp ,  AIC, log_likelyhood , BIC ,pi_steady] = ...
            em_chmm( train_obs_chmm_2 , Channel_Num_States_t , num_gmm_component_temp , max_itration , extra);
...
for tr =1:test_number
    ...
    P_Y_Model_1  = forward_backward_chmm( pi_0_1, transitions_matrices_1 ,  gmm_para_1 , Y_test );
    P_Y_Model_2 = forward_backward_chmm( pi_0_2, transitions_matrices_2   , gmm_para_2 , Y_test );
...
scores_chmm(repeat_num,:) = [F1,F2];
Label_chmm = [ones(1,test_number),zeros(1,test_number)];
...
for s = 1:1:length(state_numbers_all)
    ...
    parfor reinit_num = 1:max_reinit
        ...
        AIC_par(reinit_num,s) = AIC(end);
        pi_0_chmm_par{reinit_num,s} = pi_0_temp;
        coupling_tetha_convex_comb_par{reinit_num,s} = Coupling_Tetha_temp;
        transition_matrices_convex_par{reinit_num,s} = transitions_matrices_temp;
        chmm_gmm_para_par{reinit_num,s} = hmm_gmm_para_temp;
...
for tr =1:test_number
    ...
    Y_test{1,1}=test_obs_hmm_1{1,tr};
...
for tr =1:test_number
    ...
    Y_test{1,1} = test_obs_hmm_2{1,tr};
...
scores_hmm(repeat_num,:) = [F1,F2];


%% svm part

x = [features_train_svm_1,features_train_svm_2];
y = [ones(1,train_number),zeros(1,train_number)];

c = cvpartition(2*train_number,'KFold',5);
opts = struct('Optimizer','bayesopt','CVPartition',c,...
    'AcquisitionFunctionName','expected-improvement-plus','ShowPlots',false,'Verbose',0);

svmmod = fitcsvm(x',y(:),'KernelFunction','rbf',...
    'OptimizeHyperparameters','auto','HyperparameterOptimizationOptions',opts);

x = [features_test_svm_1,features_test_svm_2];
[est_lable,y_two_class] = predict(svmmod,x');
scores_svm(repeat_num,:) = y_two_class(:,2)';
